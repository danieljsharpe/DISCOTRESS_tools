...
printed to the "tp_distribns.dat" output file from DISCOTRESS. Namely: the first passage time (FPT) distribution, and the path length,
...
class Analyse_tp_distribns(object):
    ...
    def plot_hist(self,hist_arr,nxticks,nyticks,ymax,figfmt="pdf",color="blue",xtick_dp=0,ytick_dp=2):
...
#            plt.xlabel("$\log_{10}(t_\mathrm{FPT}\ /\ \mathrm{ns})$",fontsize=42)
            plt.xlabel("$\log_{10}(t_\mathrm{FPT})$",fontsize=42)
            plt.ylabel("$p ( \log_{10} t_\mathrm{FPT} )$",fontsize=42)
        else:
            plt.xlabel("$t_\mathrm{FPT}$",fontsize=42)
            plt.ylabel("p(t_\mathrm{FPT})$",fontsize=42)
        ax = plt.gca()
...
if __name__=="__main__":
    ...
    # 0=dynamical activity (path length), 1=time, 2=path prob, 3=entropy flow
    stat=1
    ...
    # plot
    calc_hist_obj.plot_hist(hist_arr,nxticks,nyticks,ymax,figfmt="svg",color="deeppink")
