'''
Python script to process data from DISCOTRESS: plot selected trajectories that are written to the output files walker.0.x.dat,
and analyse the time-dependent probability distributions for macrostates
...
class Analyse_trajs(object):
    ...
    def calc_tp_stats(self,n_tpbins):
        qi_vals= np.full(n_tpbins,float("nan"),dtype=float) # committor probabilities
        ri_vals = np.full(n_tpbins,0.,dtype=float) # transient visitation probabilities
